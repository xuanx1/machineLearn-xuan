...
dashboardTitleContainer.style.alignItems = "center";
dashboardTitleContainer.style.justifyContent = "space-between";
dashboardTitleContainer.style.margin = "20px";
...
// apply button
const indexApplyButton = document.createElement("img");
indexApplyButton.src = "images/ok2.svg";
indexApplyButton.alt = "Apply Indexes";
indexApplyButton.style.cursor = "pointer";
indexApplyButton.style.width = "24px";
indexApplyButton.style.height = "24px";
indexApplyButton.style.borderRadius = "50%";
indexApplyButton.style.backgroundColor = "#f67a0a";
indexApplyButton.style.padding = "5px";
indexApplyButton.style.boxShadow = "0px 1px 3px rgba(0, 0, 0, 0.3)";

// hover description for apply button
const applyButtonDescription = d3
  .select("body")
  .append("div")
  ...
  .text("Apply Index Values");

indexApplyButton.addEventListener("mouseover", (event) => {
  const rect = indexApplyButton.getBoundingClientRect();
  applyButtonDescription
    .style("left", `${rect.left - 100}px`)
    .style("top", `${rect.top - 30}px`)
    .style("display", "block");
});

indexApplyButton.addEventListener("mouseout", () => {
  applyButtonDescription.style("display", "none");
});

// Add button to the container
dashboardTitleContainer.appendChild(indexApplyButton);

// Add the container to the dashboard
dashboardContainer.appendChild(dashboardTitleContainer);
...
routeButton.addEventListener("click", async () => {
  ...
  if (originMatch && destinationMatch) {
    ...
    async function calculateCurvedPath(start, end) {
      // Create a loading bar container with percentage text
      ...
      loadingContainer.style.boxShadow = "0 0 10px rgba(0, 0, 0, 0.3)";
      ...
      loadingContainer.style.minWidth = "250px";
      ...
      loadingText.innerHTML = "Calculating optimal route...<br><br><span style='font-size: 12px; color:rgb(116, 116, 116); '>Page may become unresponsive. <br>We appreciate your patience!</span><br>";
      ...
      loadingContainer.appendChild(percentageText);
      ...
      try {
        // Start calculation with progress updates
        ...
        if (!gridData.features || gridData.features.length === 0) {
          ...
          return [start, end]; // Direct path if no grid data
        }
        updateProgress(35);
        ...
        console.log(`Direct distance between points: ${directDistance.toFixed(2)}km`);
        
        // Create a line along the direct path for reference
        const directLine = turf.lineString([start, end]);
        updateProgress(40);
        
        // Extract points with their FFI values
        const gridPoints = gridData.features.map(feature => {
          ...
          return {
            coordinates,
            ffi: feature.properties.ffi || 0.5
          };
        });
        updateProgress(50);
        
        // Define the path segments (divide the path into equal segments)
        // Increase segment count for more potential twists and turns
        const segmentCount = Math.min(24, Math.max(8, Math.floor(directDistance / 30))); 
        console.log(`Creating path with ${segmentCount} segments`);
        
        // Initialize the path with origin
        const path = [start];
        
        // Function to add irregularity to a point with varying intensity based on context
        const addIrregularity = (coordinates, baseDeviation, forceDirection = null) => {
          // Add random deviation with possible amplification
          const amplifier = Math.random() < 0.3 ? 2 + Math.random() * 3 : 1; // 30% chance of significant deviation
          const deviation = baseDeviation * amplifier * (0.5 + Math.random());
          
          // Either use forced direction or random angle
          const angle = forceDirection !== null ? forceDirection : Math.random() * Math.PI * 2;
          
          const offsetX = deviation * Math.cos(angle);
          const offsetY = deviation * Math.sin(angle);
          
          // Apply offsets
          const irregularPoint = [
            coordinates[0] + offsetX, 
            coordinates[1] + offsetY
          ];
          
          // Check if point is within SEA boundary
          if (turf.booleanPointInPolygon(turf.point(irregularPoint), seaPolygon)) {
            return irregularPoint;
          }
          
          // If outside boundary, try with half the deviation
          const reducedPoint = [
            coordinates[0] + offsetX/2, 
            coordinates[1] + offsetY/2
          ];
          
          if (turf.booleanPointInPolygon(turf.point(reducedPoint), seaPolygon)) {
            return reducedPoint;
          }
          
          // If still outside boundary, return original point
          return coordinates;
        };
        
        // Variables for creating oscillating patterns
        let oscillationPhase = Math.random() * Math.PI * 2; // Random starting phase
        let oscillationFrequency = 0.2 + Math.random() * 0.3; // Random frequency
        let oscillationAmplitude = 0.003 + Math.random() * 0.004; // Base amplitude
        let prevDirection = Math.random() * Math.PI * 2; // Track previous direction
        let turnCount = 0; // Count consecutive turns in same direction
        updateProgress(55);
        
        // Create path segments with enhanced irregularities
        for (let i = 1; i <= segmentCount; i++) {
          // Update progress based on segment completion
          const segmentProgress = 55 + ((i / segmentCount) * 35);
          updateProgress(segmentProgress);
          
          // Calculate the target point along the direct line
          const segmentRatio = i / segmentCount;
          const targetPoint = turf.along(directLine, directDistance * segmentRatio, {units: 'kilometers'});
          
          // Define search parameters for this segment
          const searchRadius = Math.max(30, directDistance * 0.2); // Increased search radius
          
          // Find points near this segment of the path
          const nearbyPoints = gridPoints.filter(pt => {
            const pointFeature = turf.point(pt.coordinates);
            ...
            // Calculate distance from this point to the target point
            const distance = turf.distance(pointFeature, targetPoint, {units: 'kilometers'});
            return distance < searchRadius;
          });
          
          if (nearbyPoints.length === 0) {
            console.log(`No valid points found for segment ${i}, using direct path point`);
            
            // For direct path points, use oscillation pattern for irregularity
            oscillationPhase += oscillationFrequency; // Advance the phase
            const oscillationDirection = oscillationPhase;
            
            // Add irregularity with oscillation
            const irregularDirectPoint = addIrregularity(
              targetPoint.geometry.coordinates, 
              oscillationAmplitude, 
              oscillationDirection
            );
            
            path.push(irregularDirectPoint);
            continue;
          }
          
          // Score points based on FFI and proximity to target point
          const scoredPoints = nearbyPoints.map(pt => {
            const distance = turf.distance(turf.point(pt.coordinates), targetPoint, {units: 'kilometers'});
            const distanceFactor = 1 - (distance / searchRadius); // Higher when closer to target
            
            // Calculate directional continuity factor
            let continuityFactor = 0;
            if (path.length > 1) {
              const prevVector = [path[path.length-1][0] - path[path.length-2][0], 
                                path[path.length-1][1] - path[path.length-2][1]];
              const newVector = [pt.coordinates[0] - path[path.length-1][0],
                                pt.coordinates[1] - path[path.length-1][1]];
              
              // Calculate direction angles
              const prevAngle = Math.atan2(prevVector[1], prevVector[0]);
              const newAngle = Math.atan2(newVector[1], newVector[0]);
              
              // Calculate angle difference (in radians)
              let angleDiff = Math.abs(newAngle - prevAngle);
              if (angleDiff > Math.PI) angleDiff = 2 * Math.PI - angleDiff;
              
              // Normalize to [0,1] where 1 is same direction, 0 is opposite
              continuityFactor = 1 - (angleDiff / Math.PI);
              
              // Update previous direction
              prevDirection = newAngle;
            }
            
            // Generate twistiness factor - sometimes we want to twist
            // Increased likelihood of twists based on turnCount
            const twistBias = turnCount < 2 ? 0.3 : -0.1; // Favor twists initially, then straighten
            const twistFactor = (Math.random() < (0.4 + twistBias)) ? 
                             Math.random() * 0.3 : -0.1; // Twist irregularity
            
            // Balance FFI value with proximity and varied continuity
            // Adjust formula to sometimes favor sharp turns
            const score = (pt.ffi * 0.4) + 
                        (distanceFactor * 0.3) + 
                        (continuityFactor * 0.1) + 
                        (twistFactor) + 
                        (Math.random() * 0.2); // Randomness
          ...
          // Pick from top points with variable selection pressure
          // Sometimes pick further down the list for more varied paths
          const variableRange = Math.min(6, scoredPoints.length);
          const pickIndex = Math.floor(Math.pow(Math.random(), 1.5) * variableRange);
          const selectedPoint = scoredPoints[pickIndex];
          
          // Update turn counter if we're making a significant turn
          if (path.length > 1) {
            const prevVector = [path[path.length-1][0] - path[path.length-2][0], 
                              path[path.length-1][1] - path[path.length-2][1]];
            const newVector = [selectedPoint.coordinates[0] - path[path.length-1][0],
                             selectedPoint.coordinates[1] - path[path.length-1][1]];
            
            // Calculate turn angle
            const prevAngle = Math.atan2(prevVector[1], prevVector[0]);
            const newAngle = Math.atan2(newVector[1], newVector[0]);
            let turnAngle = newAngle - prevAngle;
            if (turnAngle > Math.PI) turnAngle -= 2 * Math.PI;
            if (turnAngle < -Math.PI) turnAngle += 2 * Math.PI;
            
            // Check if we're continuing to turn in the same direction
            if ((turnAngle > 0 && prevDirection > 0) || (turnAngle < 0 && prevDirection < 0)) {
              turnCount++;
            } else {
              turnCount = 0;
            }
            prevDirection = turnAngle;
          }
          
          // Create dramatic twists at certain points of the route
          const isTwistPoint = Math.random() < 0.15; // 15% chance of twist points
          let irregularityMagnitude;
          
          if (isTwistPoint) {
            // Create a more dramatic twist
            irregularityMagnitude = 0.005 + Math.random() * 0.008;
            console.log(`Adding dramatic twist at segment ${i}`);
            
            // After a twist, add 1-3 follow-up points to create a meaningful detour
            const twistDirection = Math.random() * Math.PI * 2;
            const twistPoints = 1 + Math.floor(Math.random() * 3);
            
            for (let t = 0; t < twistPoints; t++) {
              // Place points along the twist direction with increasing then decreasing deviation
              const twistFactor = t === 0 ? 1 : (twistPoints - t) / twistPoints;
              const twistMagnitude = irregularityMagnitude * twistFactor * 1.5;
              
              // Create a point along the twist direction
              const basePoint = t === 0 ? selectedPoint.coordinates : path[path.length-1];
              const twistPoint = addIrregularity(basePoint, twistMagnitude, twistDirection + (Math.random() - 0.5));
              
              path.push(twistPoint);
            }
          } else {
            // Normal point - determine irregularity magnitude based on position
            // More irregularity in middle sections
            const segmentPosition = Math.min(i, segmentCount - i + 1) / segmentCount; 
            const baseIrregularity = 0.001 + (0.004 * segmentPosition);
            
            // Oscillating component for varied irregularity
            oscillationPhase += oscillationFrequency;
            const oscillationComponent = Math.sin(oscillationPhase) * oscillationAmplitude;
            irregularityMagnitude = baseIrregularity + Math.abs(oscillationComponent);
            
            // Add calculated irregularity to the selected point
            const irregularPoint = addIrregularity(
              selectedPoint.coordinates, 
              irregularityMagnitude,
              Math.random() < 0.5 ? oscillationPhase : null // Sometimes follow oscillation
            );
            
            path.push(irregularPoint);
          }
          
          console.log(`Added point for segment ${i}: FFI=${selectedPoint.ffi.toFixed(2)}, Irregularity=${irregularityMagnitude.toFixed(4)}`);
          
          // Occasionally add interpolated points for smoother curves in some areas
          // But more jagged in others for contrast
          if (!isTwistPoint && Math.random() < 0.2 && i < segmentCount) { // 20% chance
            const extraPoints = 1 + Math.floor(Math.random() * 2); // 1-2 extra points
            
            for (let e = 1; e <= extraPoints; e++) {
              const t = e / (extraPoints + 1);
              const midPoint = [
                path[path.length-2][0] * (1-t) + path[path.length-1][0] * t,
                path[path.length-2][1] * (1-t) + path[path.length-1][1] * t
              ];
              
              // Add smaller irregularity to interpolated point
              const extraIrregularity = irregularityMagnitude * 0.7;
              const extraPoint = addIrregularity(midPoint, extraIrregularity);
              
              // Insert the extra point before the current point
              path.splice(path.length-1, 0, extraPoint);
            }
            console.log(`Added ${extraPoints} interpolated points for smoother curve`);
          }
        }
        ...
        // Selective smoothing - smooth some sections but keep others jagged
        const smoothedPath = [];
        smoothedPath.push(path[0]); // Always keep start point
        
        // Define segments as either "smooth" or "jagged"
        const sectionCount = Math.floor(path.length / 6); // Divide into sections
        let smoothSection = Math.random() < 0.5; // Random starting section type
        
        for (let i = 1; i < path.length - 1; i++) {
          // Switch section type occasionally
          if (i % 6 === 0) {
            smoothSection = Math.random() < 0.6; // 60% chance to switch section type
          }
          
          if (smoothSection && Math.random() < 0.8) { // Apply smoothing in smooth sections
            // Weighted average of current point and neighbors
            const weight = 0.5 + Math.random() * 0.3; // Random weight between 0.5 and 0.8
            const smoothedPoint = [
              path[i][0] * weight + (path[i-1][0] + path[i+1][0]) * ((1-weight)/2),
              path[i][1] * weight + (path[i-1][1] + path[i+1][1]) * ((1-weight)/2)
            ];
            
            // Verify smoothed point is still within bounds
            if (turf.booleanPointInPolygon(turf.point(smoothedPoint), seaPolygon)) {
              smoothedPath.push(smoothedPoint);
            } else {
              smoothedPath.push(path[i]); // Use original if smoothed point is out of bounds
            }
          } else {
            smoothedPath.push(path[i]); // Keep unsmoothed in jagged sections
          }
        }
        ...
        console.log(`Final path generated with ${smoothedPath.length} points (including twists and turns)`);
        
        // Remove the loading container after a brief delay to show 100%
        setTimeout(() => {
          loadingContainer.remove();
        }, 500);
        ...
        loadingText.textContent = "Error calculating route";
        loadingBarFill.style.backgroundColor = "#ff3333";
        updateProgress(100);
        
        // Remove the loading container after error display
        setTimeout(() => {
          loadingContainer.remove();
        }, 2000);
    ...
    sliders.forEach((sliderId) => {
      document.getElementById(sliderId).addEventListener("input", async () => {
        // Just update the displayed values, don't redraw routes
        const valueDisplay = document.getElementById(`${sliderId.replace("-filter", "-value")}`);
        if (valueDisplay) {
          valueDisplay.textContent = document.getElementById(sliderId).value;
        }
        
        // Check if export dashboard is open and update its content
        if (isExportDashboardOpen) {
          updateExportDashboardContent();
        }
      });
    ...
    // Add click event for the apply button to update the routes
    indexApplyButton.addEventListener("click", async () => {
      const originValue = originInput.value;
      const destinationValue = destinationInput.value;

      if (!originValue || !destinationValue) {
        alert("Please enter both origin and destination before applying index values.");
        return;
      }
